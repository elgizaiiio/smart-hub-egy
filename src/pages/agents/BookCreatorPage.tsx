...
import { ArrowLeft, BookOpen, ArrowUp, Loader2 } from "lucide-react";
...
const SYSTEM = `You are an AI Book & Story Creator. Create complete books:
...
- Support Arabic, English, and other languages
Use proper formatting: headings, paragraphs, page numbers.
Always respond in the user's language.`;

const GENRES = [
  { label: "Novel", prompt: "Write a novel" },
  { label: "Educational", prompt: "Write an educational book" },
  { label: "Children's", prompt: "Write a children's story" },
  { label: "Self-Help", prompt: "Write a self-help book" },
];
    ...
    <div className="h-[100dvh] flex flex-col bg-background">
        ...
        {!hasMessages && (
          <div className="flex flex-col items-center justify-center h-full">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-sm space-y-5 w-full">
              <div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center mx-auto"><BookOpen className="w-8 h-8 text-primary" /></div>
              <h2 className="text-lg font-bold text-foreground">Book Creator</h2>
              <p className="text-sm text-muted-foreground">Create complete books and stories with AI</p>

              {/* Genre selection */}
              <div className="grid grid-cols-2 gap-2">
                {GENRES.map(g => (
                  <button key={g.label} onClick={() => setSelectedGenre(g.label)} className={`py-3 px-3 rounded-2xl border transition-colors text-left ${selectedGenre === g.label ? "bg-primary/10 border-primary/30" : "bg-card border-border/30 hover:border-primary/30"}`}>
              ...
              <div className="flex items-end gap-2 rounded-2xl border border-border/50 bg-secondary/80 px-3 py-2">
                <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMessage(`${selectedGenre ? `Genre: ${selectedGenre}. ` : ""}${input}`); } }} placeholder="Describe your book idea..." rows={2} className="flex-1 bg-transparent border-none outline-none resize-none text-sm text-foreground placeholder:text-muted-foreground/60 py-1.5" style={{ minHeight: "48px" }} />
                <button onClick={() => sendMessage(`${selectedGenre ? `Genre: ${selectedGenre}. ` : ""}${input}`)} disabled={!input.trim()} className="shrink-0 w-8 h-8 flex items-center justify-center rounded-full bg-primary text-primary-foreground disabled:opacity-20"><ArrowUp className="w-4 h-4" /></button>
              </div>
            </motion.div>
          </div>
        )}
      ...
      {hasMessages && (
        <div className="shrink-0 px-4 py-3">
          <div className="flex items-end gap-2 rounded-2xl border border-border/50 bg-secondary/80 px-3 py-2">
            <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMessage(input); } }} placeholder="Continue writing..." rows={1} className="flex-1 bg-transparent border-none outline-none resize-none text-sm text-foreground placeholder:text-muted-foreground/60 py-1.5 max-h-32" style={{ minHeight: "32px" }} />
