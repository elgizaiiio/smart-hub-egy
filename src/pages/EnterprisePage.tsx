...
import { Send, Shield, Zap, Users, Server, Headphones, Lock } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-xl border border-white/[0.08] bg-white/[0.03] text-sm text-white outline-none focus:border-primary/30 transition-colors placeholder:text-white/20"
      ...
      <div>
        ...
        <textarea
          ...
          rows={4}
          className="w-full px-4 py-3 rounded-xl border border-white/[0.08] bg-white/[0.03] text-sm text-white outline-none focus:border-primary/30 transition-colors resize-none placeholder:text-white/20"
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-40"
      >
  ...
  return (
    <div data-theme="dark" className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative overflow-hidden pt-28 pb-20 md:pt-36 md:pb-28">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_hsl(var(--primary)/0.06),transparent_60%)]" />
        <div className="relative z-10 mx-auto max-w-4xl px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="font-display text-[12vw] md:text-[5vw] font-black uppercase leading-[0.9] tracking-tighter">
              AI AT{" "}
              <span className="bg-gradient-to-r from-primary to-purple-400 bg-clip-text text-transparent">ENTERPRISE</span>{" "}
              SCALE
            </h1>
            <p className="mx-auto mt-6 max-w-2xl text-lg text-muted-foreground leading-relaxed">
              Dedicated infrastructure, priority processing, and enterprise-grade security for your organization.
            </p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {highlights.map((h, i) => (
              <motion.div
                ...
                className="rounded-2xl border border-border/50 bg-card/30 p-6 hover:border-primary/20 transition-colors"
              >
      ...
      <section className="py-16 md:py-24 border-t border-border/50">
        <div className="mx-auto max-w-2xl px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="font-display text-2xl font-bold text-foreground mb-8 text-center">Get in Touch</h2>
