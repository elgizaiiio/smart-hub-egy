...
const CharacterSwapPage = () => {
  ...
  useEffect(() => {
    supabase.from("tool_landing_images").select("image_url").eq("tool_id", "character-swap").maybeSingle()
        ...
        <AnimatePresence mode="wait">
          {step === "landing" && (
            <motion.div key="landing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative min-h-[75vh] flex flex-col items-center justify-end pb-16">
              {landingImage ? <img src={landingImage} alt="Character Swap" className="absolute inset-0 w-full h-full object-cover" /> : <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/20 via-accent/10 to-background" />}
              <div className="absolute inset-0 bg-gradient-to-t from-background via-background/70 to-transparent" />
              <div className="relative z-10 text-center px-6 space-y-5">
                <h2 className="text-3xl font-bold text-foreground tracking-tight">Character Swap</h2>
                <p className="text-sm text-muted-foreground max-w-xs mx-auto">Swap characters between photos seamlessly</p>
